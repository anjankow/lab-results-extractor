...
def text_position_equals(first: TextPosition, second: TextPosition):
    if first == None or second == None:
            return False
    return first.text == second.text and first.x1 == second.x1 and first.y2 == second.y2
...
def extract_column_from_data(data, selection, skip_words: List[str]=[], confidence=60, line_word_max_deviation=5) -> List[TextPosition]:
...
def extract_column_data(image_path, selection, skip_words: List[str]=[], confidence=60, line_word_max_deviation=5, config="--oem 3 --psm 6")->List[TextPosition]:
    ...
    return extract_column_from_data(data, selection, skip_words, confidence, line_word_max_deviation)

def get_image_data(image_path, config="--oem 3 --psm 6"):
    # Read the image using OpenCV
    image = cv2.imread(image_path)
